...
public class ShapeFactoryProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        try {
            ...
            for (Element annotatedElement : roundEnvironment.getElementsAnnotatedWith(ShapeFactory.class)) {
                //指定此注解只能使用在class上
                if (annotatedElement.getKind() != ElementKind.CLASS) {
                    //注解没有使用在class上，直接抛出异常终止编译
                    throw new ProcessingException(annotatedElement, "Only classes can be annotated with @%s",
                            ShapeFactory.class.getSimpleName());
                }

                TypeElement typeElement = (TypeElement) annotatedElement;
                //新建FactoryAnnotatedClass对象存放annotationElement信息
                com.example.shapetest.shape.FactoryAnnotatedClass annotatedClass = new com.example.shapetest.shape.FactoryAnnotatedClass(typeElement);
                //检查是否格式正确
    ...
    public Set<String> getSupportedAnnotationTypes() {
        ...
        //此处必须是完整的包名+类名
        annotation.add(ShapeFactory.class.getCanonicalName());
        return annotation;
    }
        ...
        if (superClassElement.getKind() == ElementKind.INTERFACE) {
            if (!classElement.getInterfaces().contains(superClassElement.asType())) {
                throw new ProcessingException(classElement,
                        ...
                        classElement.getQualifiedName().toString(), ShapeFactory.class.getSimpleName(),
